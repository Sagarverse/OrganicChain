...
/**
 * Generate QR code data URL for a product using only its ID as payload
...
export const downloadProductQRCode = async (
  ...
  try {
    const qrCodeDataUrl = await generateProductQRCode(productId);
